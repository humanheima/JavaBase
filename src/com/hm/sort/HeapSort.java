 ...
 * <p>
 * 参考链接：https://www.cnblogs.com/skywang12345/p/3602162.html
 * 但是算法使用递归，不使用循环
 */
public class HeapSort {
    ...
    private void minHeapify(int currentIndex, int len) {
        int leftIndex = (currentIndex * 2) + 1;//左子节点索引
        int rightIndex = leftIndex + 1;//右子节点索引
        int minIndex = leftIndex;//子节点最大索引值，默认左子节点
        if (leftIndex > len) return;//左子节点超出计算范围，直接返回
        if (rightIndex <= len && arr[rightIndex] < arr[leftIndex]) {
            //先判断右子节点那个较大
            minIndex = rightIndex;
        ...
        if (arr[minIndex] < arr[currentIndex]) {
             ...
             * 然后重新检查交换后的 maxIndex位置上的value是否符合堆特性
             */
            swap(minIndex, currentIndex);//如果父节点被子节点调换
            minHeapify(minIndex, len);//则需要继续判断换下后的父节点是否符合堆特性
        }
    }
